...
W1_Angry=pinv(X_All_Angry)*transpose(t_All_Angry);
 cvx_begin
     variables z1(800) W0_Angry(100) 
     minimize (sum(pow_pos(W0_Angry,2))+sum(z1))
...
Test0_Angry=sum(Test_Angry(:,:,1:3),3)/3;
  ...
  k5=1;
   ...
   res_Angry=transpose(W0_Angry)*transpose(test_Angry);
...
%W1 is testing Happy or Not Happy

W1_Happy=pinv(X_All_Happy)*transpose(t_All_Happy);

cvx_begin
     variables z2(800) W0_Happy(100) 
     minimize (sum(pow_pos(W0_Happy,2))+sum(z2))
      
        
     subject to
     
    p2= X_All_Happy*W0_Happy;
    
     
   transpose(t_All_Happy).*p2>=1-z2;
...
Test0_Happy=sum(Test_Happy(:,:,1:3),3)/3;
  ...
  k6=1;
   ...
   res_Happy=transpose(W0_Happy)*transpose(test_Happy);
   
  cvx_begin
     variables z3(800) W0_Neutral(100) 
     minimize (sum(pow_pos(W0_Neutral,2))+sum(z3))
      
        
     subject to
     
    p3= X_All_Neutral*W0_Neutral;
    
     
   transpose(t_All_Neutral).*p3>=1-z3;
...
Test_Neutral=imread(['.\Gabor_test\' num2str(j) '.bmp']);
Test0_Neutral=sum(Test_Neutral(:,:,1:3),3)/3;
  ...
  k7=1;
   ...
   cvx_begin
     variables z4(800) W0_Surprise(100) 
     minimize (sum(pow_pos(W0_Surprise,2))+sum(z4))
      
        
     subject to
     
    p4= X_All_Surprise*W0_Surprise;
    
     
   transpose(t_All_Surprise).*p4>=1-z4;
...
Test0_Surprise=sum(Test_Surprise(:,:,1:3),3)/3;
  ...
  k8=1;
   ...
   res_Surprise=transpose(W0_Surprise)*transpose(test_Surprise);
